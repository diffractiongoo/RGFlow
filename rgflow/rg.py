...
class BatchedRGPartition(torch.nn.Module):
    ''' RGPartition implements bijective partition of fine-grained features
        to coarse-grained and residual features.
    ...
    def split(self, x):
        ''' split x to x, z
            Input:
                x :: torch.Tensor (Batch, N, dim, *in_shape) - fine-grained features
            Output:
                x :: torch.Tensor (Batch, N, dim, *out_shape) - coarse-grained features
                z :: torch.Tensor (Batch, N, dim, *res_shape) - residual (irrelevant) features
        '''
        # assert prod(x.shape[3:]) == prod(
        #     self.in_shape), f'Data shape of x:{x.shape[3:]} does not match the designated shape {self.in_shape}.'
        x = x.view(x.shape[:3] + (-1,))  # (Batch, N, dim, :)
        x, z = x[:, :, :, self.out_indx], x[:, :, :, self.res_indx]
        x = x.view(x.shape[:3] + self.out_shape)
        return x, z
    ...
    def merge(self, x, z):
        ...
        '''
        # assert prod(x.shape[3:]) == prod(
        #     self.out_shape), f'Data shape of x:{x.shape[3:]} does not match the designated shape {self.out_shape}.'
        # assert prod(z.shape[3:]) == prod(
        #     self.res_shape), f'Data shape of z:{z.shape[3:]} does not match the designated shape {self.res_shape}.'
        x = x.view(x.shape[:3] + (-1,))  # (Batch, N, dim, :)
        indx = torch.cat([self.out_indx, self.res_indx])
        x = torch.cat([x, z], -1)
        x[:, :, :, indx] = x.clone()
...
class RGLayer(torch.nn.Module):
    ...
    def create_latent(self, x, z, **kwargs):
        x = self.partitioner.merge(x, z) # merge data
        return x

    def decode_latent(self, x, **kwargs):
        x, *rest = self.bijector(x, 1., 0., **kwargs)  # bijector backward
        return x, *rest


class RealNVP(torch.nn.Module):
    def __init__(self, nets, nett, mask, in_shape, **kwargs):
        super().__init__()
    ...
    # def split_latent(self, x, **kwargs):
    #     x, z = self.partitioner.split(x)
    #     return x, z

    def forward(self, z):
        log_det_J, x = z.new_zeros(z.shape[:2]), z
        for i in range(len(self.t)):
            x_ = x*self.mask[i]
            s = self.s[i](x_)*(~self.mask[i])
            t = self.t[i](x_)*(~self.mask[i])
            x = x_ + (~self.mask[i]) * (x * torch.exp(s) + t)
            log_det_J += s.sum(dim=self.d)
        return x, log_det_J

    def encode(self, x):
        log_det_J, z = x.new_zeros(x.shape[:2]), x
        for i in reversed(range(len(self.t))):
            z_ = self.mask[i] * z
            s = self.s[i](z_) * (~self.mask[i])
            t = self.t[i](z_) * (~self.mask[i])
            z = (~self.mask[i]) * (z - t) * torch.exp(-s) + z_
            log_det_J -= s.sum(dim=self.d)
        return z, log_det_J
